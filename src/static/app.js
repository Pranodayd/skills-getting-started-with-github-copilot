  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participants = details.participants && details.participants.length > 0
          ? details.participants.map((email) => `
              <div class="participant-row">
                <span>${email}</span>
                <button class="remove-participant-btn" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">
                  ✕
                </button>
              </div>
            `).join("")
          : "<div class='participant-row empty'>No one has signed up yet.</div>";
